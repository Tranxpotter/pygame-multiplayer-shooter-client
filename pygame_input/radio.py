...
BASIC_RADIO_SELECTION_BUTTON = ToggleButtonCircle(
    0, 0, 7, color=(255, 255, 255), outline_color=(
        100, 100, 100), outline_width=2)
...
class RadioSelection:
    def __init__(self,
                 ...
                 text_color: tuple = (0,
                                      0,
                                      0),
                 font: pygame.font.Font = pygame.font.Font(None,
                                                           25),
                 selected: bool = False,
                 button: ToggleButtonRect | ToggleButtonCircle = BASIC_RADIO_SELECTION_BUTTON,
                 active: bool = True,
                 x: int = 0,
                 y: int = 0) -> None:
    ...
    @property
    def rendered_text(self): return self.font.render(
    ...
    def button_callback(self, button: ToggleButtonRect | ToggleButtonCircle):
        self._container.on_select(self)
        button.deactivate()
    ...
    def handle_events(self, events: list, dt: float):
        ...
        if not self._container:
            raise ValueError("Radio Selection is not contained within Radio Input")
        if self.active:
            if not self.button:
...
class RadioInput:
    def __init__(self,
                 ...
                 title: str = "",
                 options: list = [],
                 arrangement: Literal["horizontal",
                                      "vertical"] = "vertical",
        ...
        for option in options:
            ...
            option._container = self
            self.options.append(option)
    
    def add_option(self, option:RadioSelection|Any, index:int|None = None):
        if not isinstance(option, RadioSelection):
            option = RadioSelection(option, str(option))
        option._container = self
        if not index:
    ...
    def set_all_text_position(
            self, text_position: Literal["top", "bottom", "left", "right"]):
        for option in self.options:
    ...
    def on_select(self, option):
        if option not in self.options:
            raise ValueError(f"{option} RadioSelection not in {self} RadioInput")
